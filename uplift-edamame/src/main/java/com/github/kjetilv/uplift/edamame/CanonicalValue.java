...
public sealed interface CanonicalValue<H extends HashKind<H>> {
    ...
    record Node<K, H extends HashKind<H>>(Hash<H> hash, Map<K, Object> value)
        implements CanonicalValue<H> {
    ...
    record Nodes<H extends HashKind<H>>(Hash<H> hash, List<?> value)
        implements CanonicalValue<H> {
    ...
    record Collision<H extends HashKind<H>>(Hash<H> hash, Object value)
        implements CanonicalValue<H> {
    ...
    record Null<K, H extends HashKind<H>>(HashKind<H> kind) implements CanonicalValue<H> {

        public static <K, H extends HashKind<H>> CanonicalValue.Null<K, H> instanceFor(HashKind<H> kind) {
            return (Null<K, H>) switch (kind) {
